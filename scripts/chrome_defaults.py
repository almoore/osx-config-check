...
write-array plugins_list enabled -bool false \
    where name -string "Shockwave Flash"
...
Examples:
    ...
    $ python chrome_defaults.py read \
    "/Users/myusername/Library/Application Support/" + \
    "Google/Chrome/Default/Preferences"

    $ python chrome_defaults.py read \
    "/Users/myusername/Library/Application Support/" + \
    "Google/Chrome/Default/Preferences" download.directory_upgrade

    $ python chrome_defaults.py write \
    "/Users/myusername/Library/Application Support/" + \
    "Google/Chrome/Default/Preferences" download.directory_upgrade -bool true

    $ python chrome_defaults.py write \
    "/Users/myusername/Library/Application Support/" + \
    "Google/Chrome/Default/Preferences" \
    plugins.plugins_list -json '[{"enabled": false, "name": "Shockwave Flash"}]'

    $ python chrome_defaults.py delete \
    "/Users/myusername/Library/Application Support/" + \
    "Google/Chrome/Default/Preferences" dns_prefetching

    $ python chrome_defaults.py write-array \
    "/Users/myusername/Library/Application Support/" + \
    "Google/Chrome/Default/Preferences" \
    plugins.plugins_list enabled -bool false \
    where name -string "Shockwave Flash"
        ...
        if args["chrome_property"] is None:
            ...
            try:
                args["value"] = get_json_field(
                    preferences_json,
                    args["chrome_property"],
                    args["preferences_filename"],
                )
                print("%s" % normalize(args["value"]))
            except KeyError:
                print(
                    "The attribute '%s' does not exist in '%s'."
...
def _recursive_write(
    ...
    dprint(
        (
            "_recursive_write: attribute_name='%s' current_attrib='%s' "
    ...
    if len(attrib_as_list) == 0:
        ...
        if delete_attrib:
            try:
                ...
                raise KeyError(
                    (
                        "Error: Attribute '%s' cannot be deleted "
            ...
            try:
                ...
                raise KeyError(
                    (
                        "Error: Attribute '%s' cannot be set because "
                ...
                if where_clause is None:
                    ...
                    if array_item[where_attrib] == where_val:
                        try:
                            ...
                            sys.exit(
                                (
                                    "Error: Attribute '%s' cannot be set "
        ...
        if not isinstance(json_obj, dict):
            raise KeyError(
                (
                    "The specified parent of the supposed sub-"
                    "attribute '%s' is not an object, and therefore "
                    "cannot have a sub-attribute."
                )
                % current_attrib
            )
        ...
        json_obj[current_attrib] = _recursive_write(
            json_obj=json_obj[current_attrib],
            attribute_name=attribute_name,
            value=value,
...
def delete_json_field(json_obj, attribute_name):
    ...
    try:
        new_json = _recursive_write(
            deepcopy(json_obj), attribute_name, value=None, delete_attrib=True
        )
        return new_json
    except KeyError:
...
def get_json_field(
    json_obj, attribute_name, json_filename=None, suppress_err_msg=False
):
    ...
    while "." in attribute_name:
        ...
        try:
            ...
            if not suppress_err_msg:
                ...
                print(
                    "Attribute '%s' not found in preferences file%s"
...
def _get_json(filename):
    try:
        ...
        sys.exit(
            (
                "File '%s' does not appear to be a valid JSON file. Check "
                "this directory for backup copies to restore to, in case "
                "this file has become corrupted."
            )
            % filename
        )
...
def get_args():
    ...
    if args["action"] == "read":
        ...
        if len(sys.argv) == 6:
            ...
            args["value"] = _get_value_and_handle_errors(
                value=write_value, type_arg=write_type
            )
        else:
            print_usage()
    elif args["action"] == "write-array":
...
def _get_value(value, type_arg):
    ...
    if type_arg == "-bool":
        ...
        try:
            ...
            raise ValueError(
                "'%s' is not a valid JSON string representation." % str(value)
            )
    else:
        raise TypeError(
            "The type '%s' is not a supported data type."
            % re.sub("-", "", str(type_arg))
        )
